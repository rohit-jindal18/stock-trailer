...
const SHORTS_MAP: Record<number, ShortInstrumentPayload> = {};
// function buildMap() {
...
export class QuickShortStrategy extends BaseStrategy {
    ...
    protected initialize(): void {
        ...
        EventEmitter.addListener(EVENT_TYPE.QS_UPDATE, this.listenToInstruments);
    ...
    buildShortMap = () => {
        for (let short of this.instruments) {
            SHORTS_MAP[short.instrumentId] = short;
        }
    }

    listenToInstruments(instruments: ShortInstrumentPayload[]) {
        console.log("iniutialz 2", instruments);
        this.instruments = instruments;
        this.buildShortMap();
        console.log("SHORTMAP", SHORTS_MAP, instruments);
        (this.instruments || []).forEach((instrument: ShortInstrumentPayload) => {
