...
export class TradingController implements ITradeDelegate, ITickerListener {
    ...
    constructor(mode?: TPMode) {
        this.initialize(mode);
    }
    

    private getAnalyzerForMode(mode: TPMode): BaseAnalyzer | undefined {
        switch (mode) {
    ...
    private async initialize(mode?: TPMode) {
        try {
            const data = fs.readFileSync(ACCESS_TOKEN_FILE_PATH, { encoding: 'utf-8' });
            const { accessToken, date } = JSON.parse(data);
            if (accessToken && date) {
                ...
                if (isDateEqual(currentDate, date)) {
                    ...
                    console.log('Access Token has expired. Login again and generate new token');
                    return Promise.reject();
                }
            } else {
                console.log('Could not read accessToken data from file');
                return Promise.reject();
            }
