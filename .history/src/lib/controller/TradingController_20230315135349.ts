...
export interface ITradeDelegate {
    ...
    getProcessedInstruments(exchange: Exchange): Promise<Record<string, Instrument>>
}
...
class TradingController implements ITradeDelegate, ITickerListener {
    ...
    public getProcessedInstruments = async (exchange: Exchange): Promise<Record<string, Instrument>> => {
        console.log("123");
        const instruments = await this.getInstruments(exchange);
        console.log("insuremtn", JSON.stringify(instruments));
        return instruments.reduce((map, instrument, index) => ({
            ...map,
    ...
    public initialize = async (userInfo: StockTrailerUser, mode?: TPMode) => {
        console.log("asd", userInfo?.session?.access_token);
        try {
            if (userInfo?.session?.access_token) {
                this
                    .accessToken = userInfo?.session?.access_token;
            } else {
        ...
        console.log("rtad", this.tradingModule);
    ...
    getInstruments = (exchange: Exchange): Promise<Instrument[]> => {
        console.log("traddd", this.tradingModule, this.tradingModule?.getInstruments(exchange));
